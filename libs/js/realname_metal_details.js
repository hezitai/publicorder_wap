(function($, _C, _W) {
	_W.realnameMetalDetail = (function() {
		var _v = {},
			_f = {
				init: function() {
				...
				getDetail: function() {
					_a.getgoodsInfo(function(result) {
						_v.personId = result.ds_person_crmId;
						$('#fromName').val(result.ds_person_name);
						$('#fromId').val(result.ds_person_id);
						$('#fromPhone').val(result.ds_person_phone);
						$('#name').val(result.ds_name);
						$('#describe').val(result.ds_information);
						$('#weight').val(result.ds_weight);

						if (result.intoDate) {
							_f.setDate($('#date'), result.intoDate);
				...
				setDate: function(elementId, str) {
					elementId.val(_C.formatting.ExDate('YYYY-MM-DD HH:mm:ss', str));
				},
